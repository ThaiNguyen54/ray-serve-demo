...
class GenerateRequest(BaseModel):
    ...
    temparature: Float that controls the randomness of the sampling. Lower
        values make the model more deterministic, while higher values make
        the model more random. Zero menas greedy sampling.
...
class GenerateResponse(BaseModel):
    ...
    output: Model output
    finish_reason: Reason the generation has finished
    """
    output: Optional[str]
...
@serve.deployment(name="VLLInference", num_replicas=1, ray_actor_options={"num_gpus": 1.0})
@serve.ingress(app)
class VLLMInference:
    ...
    def _prepare_tokenizer(self):
        ...
        if self.args.trust_remote_code:
            tokenizer = Autokenizer.from_pretrained(self.args.model, trust_remote_code=True)
        else:
    ...
    async def generate_text(self, request: GenerateRequest, raw_request: Request) -> GenerateResponse:
        logging.infor(f"Received request: {request}")
        try:
            generation_args = request.dict(exclude={'prompt', 'message'})
    ...
    async def _abort_request(self, requst_id) -> None:
        await self.engine.abort(request_id)
